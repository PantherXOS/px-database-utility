...
def get_cl_arguments():
    ...
    if args.operation == 'RESTORE':
        if args.database is None or args.database == 'SELECT':
            log.error('You need to define a target name `--database DATABASE` for restore operations.')
            sys.exit(1)
        else:
            if args.database.endswith('.dmp'):
                log.error('Use the --file flag for the absolute path; use the --database flag for the target database name.')
                sys.exit(1)
